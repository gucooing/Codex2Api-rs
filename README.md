...
管理端进入 **账户详情 → 状态复用（实验）**，可按账户开启、指定模型，并设置缓存 TTL、提前续期和失败冷却时间。
默认关闭；配置和缓存保存在 SQLite，账号之间不共享。页面显示缓存状态、剩余期限、最近探针结果、注入次数及客户端 state 优先次数，支持刷新状态与清除缓存。

启用后，匹配模型的正常 HTTP Responses 请求会优先使用客户端携带的 `x-codex-turn-state`；
请求响应返回的合格 state 也会按账号和模型缓存，供后续请求使用。没有正常 state 时请求照常发送，
不会为了探针阻塞 20–30 秒；探针只作为后续的辅助采集手段。
默认 TTL 60 分钟、提前 10 分钟续期、失败冷却 5 分钟；探针产生的额外上游调用不进入用户请求用量列表。
WebSocket 不参与实验，其他端点保持原有行为。已有客户端 state 优先透传。

跨 turn 复用是对官方协议的显式实验例外；292 字符 / 10 块仅是经验格式规则，不代表质量等级，也不保证改善回答。
