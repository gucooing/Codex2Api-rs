...
- 没有有效缓存时，客户端请求头携带的合格 state 会立即缓存并随当前请求发送。
- 正常上游 HTTP 200 响应头中的合格 state 也会立即缓存，无需等待 SSE 完成。
- 一旦有有效缓存，后续匹配请求统一使用缓存，覆盖客户端携带的任何 state。
- 正常响应带来时间更新的合格 state 时更新缓存；缺失、非目标、过期或时间更旧的 state 不会覆盖有效缓存。
- 缓存过期后恢复正常透传与采集。TTL 默认 60 分钟，按 state 自带时间戳计算并预留 30 秒。
