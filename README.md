...
管理端进入 **账户详情 → 状态复用（实验）**，按账户开启、指定模型并设置缓存 TTL。
默认关闭；配置和缓存保存在 SQLite，按账户、模型隔离。

只从正常 HTTP Responses 流量采集 `x-codex-turn-state`，不发送探针或额外请求：

- 客户端请求头携带的合格 state 原样透传，并允许更新缓存，即使已有有效缓存也不会强制替换。
- 正常上游 HTTP 200 响应头中的合格 state 也会立即缓存，无需等待 SSE 完成。
- 客户端 state 缺失、不符合目标条件或过期时，才使用同账户、同模型的有效缓存替换。
- 正常客户端请求或响应带来时间更新的合格 state 时更新缓存；缺失、非目标、过期或时间更旧的 state 不会覆盖有效缓存。
- “保活”仅靠正常流量的新 state 更新缓存，不延长旧 state 的有效期。缓存过期后恢复正常透传与采集。TTL 默认 60 分钟，按 state 自带时间戳计算并预留 30 秒。

页面显示缓存来源、采集时间、有效期、实际观察到的请求和响应次数、采集结果、长度和块数、缓存使用次数。
HTTP 请求成功但没有 state 头时会明确显示原因，支持手动刷新和清除缓存。
WebSocket 及其他端点不参与。保存配置、重新授权、身份或代理变更会清除旧缓存。

跨 turn 复用是对官方协议的显式实验例外；10 块仅是本地格式筛选规则，不代表质量等级，也不保证改善回答。
详细行为和验证范围见 [实施说明](docs/TURN_STATE_REUSE_PLAN.md)。
